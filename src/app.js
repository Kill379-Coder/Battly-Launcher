...
async function initializeAnalytics() {
  try {
    ...
    const apiUrl = 'https://api.battly.site/api/analytics';
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    if (store.get("launchboost")) {
      fetch(
        "https://api.battly.site/battlylauncher/launcher/config-launcher/config.json"
      )
        .then(async (res) => {
...
ipcMain.handle("submit-error-report", async (_event, reportData) => {
  try {
    ...
    const apiUrl = "https://api.battly.site/api/error-reports";
...
ipcMain.on("new-status-discord", async () => {
  client.login({ clientId: "1459567780050501713" });
  client.on("ready", () => {
    client
...
const pkgVersion = async () => ({ version: "1.0.1", buildVersion: 1002 });
ipcMain.handle("update-new-app", async () => {
  console.log(await pkgVersion());
  return new Promise(async (resolve, reject) => {
    fetch("https://api.battly.site/battlylauncher/launcher/config-launcher/config.json")
      .then(async (res) => {
        let data = await res.json();
